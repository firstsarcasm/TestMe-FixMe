...
1. GET /set-amount  
   ...
   ```json
   {  
    "result":"Your amount of money is 10.0"  
   }  
   ```  
---
2. GET /get-amount   
...
5. POST /register
   ...
   | Название      | Тип       | Обязательность       |                 Ограничения                  |
   | :-----------: |:---------:|:--------------------:|:--------------------------------------------:|
   | username      | String    | да                   |   Длина от 5 до 500 символов включительно    |
   | pwd           | String    | да                   | Длина от 5 до 500 символов включительно      |
   | email         | String    | да                   |          Стандартный формат email'a          |
   ```
     Пример тела запроса:
   ```
   ```json
